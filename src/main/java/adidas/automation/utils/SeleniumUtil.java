package adidas.automation.utils;
...
public final class SeleniumUtil {
	 ...
	 * @param data is the css property
	 * @return {@link String}
	 */
	public static String getCSSProperty(final List<WebElement> list, final String data) {

		return list.stream().filter(x -> x.getText().equalsIgnoreCase(data)).map(x -> x.getCssValue("background-color"))
				.findFirst().get();

	}

	/**
	 * @param cellData {@linkplain List}
	 * @param data     is the css property
	 * @return {@link String}
	 */

	public static String getData(final List<WebElement> list, final String data) {

		return list.stream().filter(x -> x.getText().equalsIgnoreCase(data)).findFirst().get().getText();
